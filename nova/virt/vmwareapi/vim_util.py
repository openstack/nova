...
def deserialize_object(factory, ser, typename):
    if ser is None:
        return ser

    if typename == "ManagedObjectReference":
        # Special case: It is a simpleContent object (the only in the wsdl)
        return vutil.get_moref(ser["value"], ser["_type"])

    typename = ser.get("class", typename)
    ...
    for child, _ in sxtype.children():
        ...
        if isinstance(v, list):
            if not v or not isinstance(v[0], dict):
                ...
                type_ = child.type[0]
                deser = [deserialize_object(factory, i, type_) for i in v]
        elif isinstance(v, dict):
            type_ = child.type[0]
            deser = deserialize_object(factory, v, type_)
        else:
            deser = v

        setattr(obj, child.name, deser)
    return obj
