...
class CreateserverextTest(test.TestCase):
    ...
    def test_create_instance_with_security_group_empty_list_json(self):
        security_groups = []
        ...
        self.assertEquals([{'name': "default"}], res_dict['server']['security_groups'])

    def test_create_instance_with_security_group_non_existing_json(self):
        security_groups = ['non-existing']
        self.stubs.Set(nova.db, 'security_group_get_by_name',
                       return_security_group_non_existing)
        body_dict = self._create_security_group_request_dict(security_groups)
        request = self._get_create_request_json(body_dict)
        response = request.get_response(fakes.wsgi_app())
        self.assertEquals(response.status_int, 400)
    ...
    def test_get_server_by_id_verify_security_groups_xml(self):
        ...
        security_groups = server.getElementsByTagName('security_groups')[0]
        security_group = security_groups.getElementsByTagName('security_group')[0]
        self.assertEqual(INSTANCE['security_groups'][0]['name'], security_group.getAttribute("name"))
