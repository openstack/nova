...
class QuotaSetsPolicyTest(base.BasePolicyTest):
    ...
    def setUp(self):
        ...
        # Check that admin is able to get another project's quota.
        self.admin_get_authorized_contexts = [
            self.legacy_admin_context, self.system_admin_context,
            self.project_admin_context]
        # Check that non-admin is not able to get another project's quota.
        self.admin_get_unauthorized_contexts = [
            self.system_member_context, self.system_reader_context,
            self.system_foo_context, self.project_member_context,
            self.project_reader_context, self.project_foo_context,
            self.other_project_member_context,
    ...
    def test_detail_quota_sets_policy(self, mock_get):
        ...
        self.common_policy_check(self.admin_get_authorized_contexts,
                                 self.admin_get_unauthorized_contexts,
                                 rule_name,
                                 self.controller.detail,
                                 self.req, 'try-other-project')
        # Check if everyone (owner) is able to get their own quota
        for cxtx in self.everyone_authorized_contexts:
